...
public class DebugScene3 {
	...
	public static void main(String[] args) {
		...
		Road roadRawModel = new Road(loader, roadWaypoints, 10, 20, 5f, 0f, heightGenerator, false);// TODO
		Entity road = setupRoad(loader, heightGenerator, roadWaypoints, roadRawModel);
		
		//heightGenerator.updateHeight(roadRawModel.getTrajectory(), x -> x <= 10f ? 1f : 1 - Math.min((x - 10f) / 20f, 1f));
		heightGenerator.updateHeight(roadRawModel.getTrajectory(), x -> x <= 50f ? 1f : 1 - Math.min((x - 50f) / 100f, 1f)); // TODO
	...
	private static List<Entity> terrainVerticesGrid(TexturedModel fern, IHeightGenerator heightGenerator, float width,
		...
		for(int z = 0; z < 50; z++) {
			for(int x = 0; x < 50; x++) {
				...
				gridElems.add(new Entity(fern, new Vector3f(xcoord, height, zcoord), 0, 0, 0, 5f));
			}
		}
