...
public class DebugHeightMapScene2 {
	...
	public static void main(String[] args) {
		...
		distanceToLODLevel.put(1000f, 0);
		...
		lodLevelToVertsPerUnit.put(1, 0.0025f);
		...
		Camera camera = new FloatingCamera(new Vector3f(0, 500.0f, 0));
		...
		TerrainLODGrid terrainLODGrid = new TerrainLODGrid(distanceToLODLevel, lodLevelToVertsPerUnit, patchSize, texWidth, texDepth,
				...
				Optional.of(Globals.getThreadPool())); // TODO
				//Optional.empty());
...
//				new Point2D.Float(camera.getPosition().x, camera.getPosition().z));
	...
	//	pathfinder.findModifierTrajectories(0.0f).ifPresent(ts -> ts.forEach(t -> placer.addNoTreeZone(t, 7.0f)));
		...
		final float terrainLODTolerance = 0f; // TODO
		...
		while(!Display.isCloseRequested()) {
			...
			entities.add(new Entity(chestnutTrunk, new Vector3f(domainLowerLeftLimit.getX(), 0, domainLowerLeftLimit.getZ()), 0f, 0f, 0f, 1000f));
			entities.add(new Entity(chestnutTrunk, new Vector3f(domainUpperRightLimit.getX(), 0, domainLowerLeftLimit.getZ()), 0f, 0f, 0f, 1000f));
			entities.add(new Entity(chestnutTrunk, new Vector3f(domainLowerLeftLimit.getX(), 0, domainUpperRightLimit.getZ()), 0f, 0f, 0f, 1000f));
			entities.add(new Entity(chestnutTrunk, new Vector3f(domainUpperRightLimit.getX(), 0, domainUpperRightLimit.getZ()), 0f, 0f, 0f, 1000f));
			...
			//terrainPatches.forEach(tp -> renderer.processTerrain(tp));
